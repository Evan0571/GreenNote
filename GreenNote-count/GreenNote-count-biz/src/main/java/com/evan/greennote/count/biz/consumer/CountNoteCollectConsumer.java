...
import com.evan.greennote.count.biz.enums.CollectUnCollectNoteTypeEnum;
import com.evan.greennote.count.biz.model.dto.CountCollectUnCollectNoteMqDTO;
...
@Component
@RocketMQMessageListener(consumerGroup = "greennote_group_"+ MQConstants.TOPIC_COUNT_NOTE_COLLECT,
...
public class CountNoteCollectConsumer implements RocketMQListener<String> {
    ...
    private void consumeMessage(List<String> bodies) {
        log.info("==> [笔记收藏]聚合消息，size: {}", bodies.size());
        log.info("==> [笔记收藏]聚合消息，{}", JsonUtils.toJsonString(bodies));
        ...
        rocketMQTemplate.asyncSend(MQConstants.TOPIC_COUNT_NOTE_COLLECT_2_DB, message, new SendCallback() {
            ...
            public void onSuccess(SendResult sendResult) {
                log.info("==> [计数服务：笔记收藏数入库] MQ发送成功，SendResult:{}", sendResult);
            }
            @Override
            public void onException(Throwable throwable) {
                log.error("==> [计数服务：笔记收藏数入库] MQ发送失败", throwable);
            }
        });
    }
}
